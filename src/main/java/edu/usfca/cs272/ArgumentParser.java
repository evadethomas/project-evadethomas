...
public class ArgumentParser {
	...
	public void parse(String[] args) {
		...
		for (int i = 0; i < args.length - 1; i++) {
			...
			if (isFlag(args[i])) {
				String flag = args[i];
				i++;
				//once a flag is found, increment i to check the next arg. If arg is not a flag, add it to dict. Otherwise add null.
				if (!isFlag(args[i])) {
					map.put(flag, args[i]);
				} else {
					map.put(flag, null);
					//decrement i if the next arg is a flag to start the loop in the correct place to get another pair
					i--;
				}
			}
		}
		//Handles the edge case where the last arg is a flag with no value corresponding to it
	...
	public Path getPath(String flag, Path backup) {
		...
		String val = map.get(flag);
		if (val != null) {
			Path test = Path.of(map.get(flag));
			return test;
		} else {
			return backup;
		}
